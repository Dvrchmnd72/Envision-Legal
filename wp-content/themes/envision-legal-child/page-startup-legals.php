...
<section class="el-section el-section--cream" id="sl-enquiry-form" aria-labelledby="sl-form-heading">
	<div class="el-container" style="max-width:680px;">
		...
		<?php
		if ( isset( $_POST['sl_form_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sl_form_nonce'] ) ), 'sl_intake_form' ) ) {
			$name  = sanitize_text_field( wp_unslash( $_POST['sl_name'] ?? '' ) );
			$email = sanitize_email( wp_unslash( $_POST['sl_email'] ?? '' ) );
			$phone = sanitize_text_field( wp_unslash( $_POST['sl_phone'] ?? '' ) );
			$stage = sanitize_text_field( wp_unslash( $_POST['sl_stage'] ?? '' ) );
			$need  = sanitize_text_field( wp_unslash( $_POST['sl_need'] ?? '' ) );
			$notes = sanitize_textarea_field( wp_unslash( $_POST['sl_notes'] ?? '' ) );
			$to      = 'hello@envisionlegal.com.au';
			$subject = 'New Startup Legals Enquiry — ' . $name;
			$body    = "New startup legals enquiry from the website:\n\n"
				. "Name: $name\n"
				. "Email: $email\n"
				. "Phone: $phone\n"
				. "Startup stage: $stage\n"
		...
		<form method="post" action="#sl-enquiry-form" novalidate>
			<?php wp_nonce_field( 'sl_intake_form', 'sl_form_nonce' ); ?>
