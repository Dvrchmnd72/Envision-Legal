...
<section class="el-section el-section--cream" id="fgc-enquiry-form" aria-labelledby="fgc-form-heading">
	<div class="el-container" style="max-width:680px;">
		...
		<?php
		if ( isset( $_POST['fgc_form_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['fgc_form_nonce'] ) ), 'fgc_intake_form' ) ) {
			$name    = sanitize_text_field( wp_unslash( $_POST['fgc_name'] ?? '' ) );
			$email   = sanitize_email( wp_unslash( $_POST['fgc_email'] ?? '' ) );
			$phone   = sanitize_text_field( wp_unslash( $_POST['fgc_phone'] ?? '' ) );
			$company = sanitize_text_field( wp_unslash( $_POST['fgc_company'] ?? '' ) );
			$size    = sanitize_text_field( wp_unslash( $_POST['fgc_size'] ?? '' ) );
			$spend   = sanitize_text_field( wp_unslash( $_POST['fgc_spend'] ?? '' ) );
			$notes   = sanitize_textarea_field( wp_unslash( $_POST['fgc_notes'] ?? '' ) );
			$to      = 'hello@envisionlegal.com.au';
			$subject = 'New Fractional GC Enquiry from ' . $name;
			$body    = "New Fractional General Counsel enquiry:\n\nName: $name\nEmail: $email\nPhone: $phone\nCompany: $company\nSize: $size\nSpend: $spend\nNotes:\n$notes\n";
		...
		<form method="post" action="#fgc-enquiry-form" novalidate>
			<?php wp_nonce_field( 'fgc_intake_form', 'fgc_form_nonce' ); ?>
			<div class="sha-form-group">
